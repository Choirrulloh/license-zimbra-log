...
async function migrate() {
  ...
  try {
    ...
    const columns = await db.all("PRAGMA table_info(code_obfuscations)");
    ...
    // Check if table needs recreation (check if it has all new fields)
    const hasLicenseInjected = columns.some(col => col.name === 'license_injected');

    if (!hasLicenseInjected) {
      console.log('Table needs recreation to add new fields...');
      ...
      await db.run(`
        ...
        SELECT id, user_id, ${hasCustomerId ? 'customer_id' : 'NULL as customer_id'}, original_filename, file_size, file_hash,
               obfuscation_level, options, obfuscated_filename, obfuscated_size, output_path,
               status, error_message, 0 as license_injected, NULL as injected_product_id, NULL as validation_options,
      ...
      console.log('✓ Table already has all required fields, skipping recreation...');
    }

    // Recreate indexes (safe to run multiple times with IF NOT EXISTS)
